...
const metricsSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(
        fetchMetric.fulfilled,
        (state, action: PayloadAction<MetricDataPoint[]>) => {
          const metric = (action.meta.arg as { metric: string }).metric;
      ...
      .addCase(fetchMetric.rejected, (state, action) => {
        state.loading = false;
        state.error = action.payload || 'Error fetching metric';
      });
  },
});
